...
async function main() {
  ...
  // Deploy FHEBank
  console.log("\n🏦 Deploying FHEBank...");
  const FHEBank = await ethers.getContractFactory("FHEBank");
  ...
  const tx = await fheVoting.createVotingSession(
    "Sample Proposal",
    "This is a sample proposal for testing the FHEVoting contract",
  ...
  fs.writeFileSync(
    path.join(deploymentsDir, "hardhat-deployment.json"),
